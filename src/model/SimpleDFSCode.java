...
public class SimpleDFSCode extends ArrayList<DFS> {
  private static final long serialVersionUID = 1L;
  private ArrayList<Integer> rmPath;

  public SimpleDFSCode() {
  ...
  public void toGraph(Graph g) {
    ...
    for (DFS it : this) {
      ...
      if (it.fromLabel != -1) g.get(it.from).label = it.fromLabel;
      if (it.toLabel != -1) g.get(it.to).label = it.toLabel;

      g.get(it.from).push(it.from, it.to, it.eLabel);
      if (!g.directed) g.get(it.to).push(it.to, it.from, it.eLabel);
    }
  ...
  public ArrayList<Integer> buildRMPath() {
    ...
    for (int i = size() - 1; i >= 0; --i) {
      if (this.get(i).from < this.get(i).to
          && // forward
  ...
  public int countNode() {
    ...
    for (DFS it : this) nodeCount = Math.max(nodeCount, Math.max(it.from, it.to) + 1);
