...
public class KMeans {
  ...
  // output
  private double[][]
      centroids; // position vectors of centroids                      dim(2): (k) by (number of
  // channels)
  private int[]
      assignment; // assigns each point to nearest centroid [0, k-1]    dim(1): (number of pixels)
  ...
  /**
   * Empty constructor is private to ensure that clients have to use the Builder inner class to
  ...
  public void run() {
    ...
    for (int n = 0; n < iterations; n++) {
      // System.out.println("cluster before");
      cluster();
      // System.out.println("cluster after");
  ...
  public void updateStep() {
    ...
    for (int i = 0; i < k; i++) for (int j = 0; j < n; j++) centroids[i][j] = 0;
    ...
    for (int i = 0; i < m; i++) {
      ...
      for (int j = 0; j < n; j++) centroids[assignment[i]][j] += points[i][j];
    }
  ...
  public boolean stop(double prevWCSS) {
    ...
    else return prevWCSS == WCSS; // TODO: make comment (more exact, but could be much slower)
  ...
  /**
   * Signals to stop running KMeans when the marginal improvement in WCSS from the last step is
   * small.
   *
   * @param prevWCSS error from previous step in the run
   * @return
