...
public class GreedySelector {
  ...
  public ArrayList<SimpleDFSCode> selectTopKPatterns(ArrayList<SimpleDFSCode> candidates, int k) {
    ...
    for (SimpleDFSCode dfsCode : candidates) {
      ...
      System.out.println(
          "Processing pattern with first edge: "
              + firstDFS.fromLabel
              + "-["
              + firstDFS.eLabel
              + "]->"
  ...
  private boolean isPatternMatch(SimpleDFSCode dfsCode, int startNode) {
    ...
    for (Edge e : TRANS.get(startNode).edge) {
      if (e.eLabel == firstDFS.eLabel && TRANS.get(e.to).label == firstDFS.toLabel) {
  ...
  private boolean matchRemainingEdges(
      SimpleDFSCode dfsCode, int currentEdgeIndex, int currentNode, Set<Integer> visited) {
  ...
  public String patternToString(SimpleDFSCode pattern) {
    ...
    for (int i = 0; i < pattern.size(); i++) {
      ...
      sb.append(edge.fromLabel).append("-[").append(edge.eLabel).append("]-").append(edge.toLabel);
  ...
  private void addRemainingMatchedNodes(
      SimpleDFSCode pattern, int edgeIndex, int currentNode, Set<Integer> visited) {
