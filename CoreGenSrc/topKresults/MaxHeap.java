...
public class MaxHeap<NodeType, EdgeType> {
  ...
  public void initOriginList(
      List<HashMap<SearchLatticeNode<NodeType, EdgeType>, Frequency>> orginList) {
  ...
  protected void adjustUp(int start) {
    ...
    HashMap<SearchLatticeNode<NodeType, EdgeType>, Frequency> currentIndexMap =
        heap.get(currentIndex);
    OUT:
    while (currentIndex > 0) {
      HashMap<SearchLatticeNode<NodeType, EdgeType>, Frequency> parentIndexMap =
          heap.get(parentIndex);
      for (Entry<SearchLatticeNode<NodeType, EdgeType>, Frequency> currentIndexentry :
          currentIndexMap.entrySet()) {
        for (Entry<SearchLatticeNode<NodeType, EdgeType>, Frequency> parentIndexentry :
            parentIndexMap.entrySet()) {
  ...
  private void adjustDown(int index) {
    ...
    while (leftChildIndex < size) {
      ...
      for (Entry<SearchLatticeNode<NodeType, EdgeType>, Frequency> entryl : left.entrySet()) {
        for (Entry<SearchLatticeNode<NodeType, EdgeType>, Frequency> entryt :
            currentIndexMap.entrySet()) {
          int maxIndex = 0;
          if (right != null) {
            for (Entry<SearchLatticeNode<NodeType, EdgeType>, Frequency> entryr :
                right.entrySet()) {
              maxIndex =
                  (entryl.getValue().compareTo(entryr.getValue()) >= 0
                      ? leftChildIndex
                      : rightChildIndex);
            }
