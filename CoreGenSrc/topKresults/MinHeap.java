/**
 * Project Name:frequentPatternsMiningV1.4 File Name:MinHeap.java Package Name:topKresults Date:Mar
 * 28, 2019 Copyright (c) 2019, zengjian29@126.com All Rights Reserved.
 */
package topKresults;

/**
 * ClassName:MinHeap Function: TODO ADD FUNCTION. Reason: TODO ADD REASON. Date: Mar 28, 2019
 *
 * @author JIAN
 * @version
 * @since JDK 1.6
...
public class MinHeap<NodeType, EdgeType> {
  ...
  public IntFrequency getMinFrequency() {
    ...
    if (size < maxSize) {
      ...
      for (Entry<SearchLatticeNode<NodeType, EdgeType>, Frequency> entry0 :
          minHeapList.get(0).entrySet()) {
   ...
   */
  public Boolean addTopk(SearchLatticeNode<NodeType, EdgeType> pattern, Frequency frequency) {
    int size = minHeapList.size();
    int maxSize = Settings.k;
    HashMap<SearchLatticeNode<NodeType, EdgeType>, Frequency> midMap =
        new HashMap<SearchLatticeNode<NodeType, EdgeType>, Frequency>();
    if (size < maxSize) {
      ...
      for (Entry<SearchLatticeNode<NodeType, EdgeType>, Frequency> entry : rootMap.entrySet()) {
        ...
        if (frequency.compareTo(root) > 0) { // if the frequency is larger than root
          ...
          HashMap<SearchLatticeNode<NodeType, EdgeType>, Frequency> addMap =
              new HashMap<SearchLatticeNode<NodeType, EdgeType>, Frequency>();
  ...
  private static <NodeType, EdgeType> void heapUp(
      int pCount, List<HashMap<SearchLatticeNode<NodeType, EdgeType>, Frequency>> dataList) {
  ...
  private static <NodeType, EdgeType> void heapDown(
      int i, List<HashMap<SearchLatticeNode<NodeType, EdgeType>, Frequency>> dataList) {
    int left = 2 * (i + 1) - 1;
    int right = 2 * (i + 1);
    ...
    if (left < dataList.size() && right < dataList.size()) {
      ...
      for (Entry<SearchLatticeNode<NodeType, EdgeType>, Frequency> entryl : leftMap.entrySet()) {
        for (Entry<SearchLatticeNode<NodeType, EdgeType>, Frequency> entryr : rightMap.entrySet()) {
          for (Entry<SearchLatticeNode<NodeType, EdgeType>, Frequency> entrys :
              smallestMap.entrySet()) {
            if (entryl.getValue().compareTo((entryr.getValue())) < 0) { // left smaller than right
              if (entrys.getValue().compareTo((entryl.getValue())) <= 0) {
                return;
              } else if (entrys.getValue().compareTo((entryl.getValue()))
                  > 0) { // choose left child
                smallest = left;
              }
            } else if (entryl.getValue().compareTo(entryr.getValue())
                >= 0) { // right smaller than left
  ...
  // i<j
  private static <NodeType, EdgeType> void swap(
      int i, int j, List<HashMap<SearchLatticeNode<NodeType, EdgeType>, Frequency>> dataList) {
    List<HashMap<SearchLatticeNode<NodeType, EdgeType>, Frequency>> midList =
        new ArrayList<HashMap<SearchLatticeNode<NodeType, EdgeType>, Frequency>>();
