...
public class JoinMNI<NodeType, EdgeType> {
  ...
  public void joinMNIPlus(Frequency threshold) {
    ...
    HashMap<Integer, PatternEdges> edgesMap =
        (HashMap<Integer, PatternEdges>) allEdgeTables.clone();
  ...
  public InsTable firstJoinPass() {
    ...
    while (reducedTables != null && reducedTables.size() != 0) {
      ...
      if (adjEdge == null || ReTable.getSize() == 0) break;
  ...
  public InsTable easyTupleReduce() {
    ...
    for (Entry<Integer, PatternEdges> pesEntry : qry.getPEMap().entrySet()) {
      ...
      pesMap.putAll(
          (HashMap<Integer, PatternEdges>) qry.getPEMap().clone()); // <edgeIdx, patternEdges>
      ...
      while (pesMap != null && pesMap.size() != 0) {
        ...
        if (adjEdge == null || sampleTable.getSize() == 0) break;
  ...
  public int traverseTable(
      PatternEdges table,
      int tau,
      int domSize,
      HashMap<Integer, PatternEdges> allEdgeTables,
    ...
    while (domSize < tau && table.unTraversedSet().size() != 0) {
      ...
      HashMap<Integer, PatternEdges> midMap =
          (HashMap<Integer, PatternEdges>) allEdgeTables.clone();
      ...
      while (midMap != null && midMap.size() != 0) {
        ...
        if (adjEdge == null || RTable.getSize() == 0) break;
        midMap.remove(adjEdge.getEdgeIdx());
        //				System.out.println("join tables:"+RTable.getPatNodes()+" -
        // ["+adjEdge.getIndexA()+","+adjEdge.getIndexB()+"]");
        //				System.out.println("RTable size:"+RTable.getSize()+" adjEdge
        // size:"+adjEdge.getNodePairs().size());
        // full join
        joinCount++;
    ...
    while (it.hasNext()) {
      ...
      for (int i = 0; i < qrySize; i++) {
        if (domainMap.get(i) == null) {
          ...
          domainMap.put(i, dom);
        } else domainMap.get(i).add(ins.getNode(i));
      }
    }
   ...
   */
  private PatternEdges getAdjacentTable(
      InsTable curInsTable, HashMap<Integer, PatternEdges> edgesMap) {
  ...
  private InsTable priorJoinTables(InsTable interTable, PatternEdges adjEdge) {
    ...
    if (pNodeSet.contains(adjEdge.getIndexA()) && pNodeSet.contains(adjEdge.getIndexB())) {
      ...
      for (int i = 0; i < interTable.getSize(); i++) {
        ...
        String curInter =
            interIns.getNode(adjEdge.getIndexA()) + "_" + interIns.getNode(adjEdge.getIndexB());
        Iterator adjIt = adjNodePairs.iterator();
        while (adjIt.hasNext()) {
          int[] adjArr = (int[]) adjIt.next();
          if (interNode < adjArr[pairColumnA]) break;
          if (interNode > adjArr[pairColumnA]) continue;
      ...
      for (int i = 0; i < interTable.getSize(); i++) {
        ...
        while (it.hasNext()) {
          ...
          if (interNode < adjJoinNode) break;
          if (interNode > adjJoinNode) continue;
   ...
   */
  @SuppressWarnings("unchecked")
  private InsTable fullJoinTables(
      InsTable interTable, PatternEdges adjEdge, int joinCount, int tau) {
    ArrayList<int[]> adjNodePairs = adjEdge.getNodePairs();
    ...
    if (pNodeSet.contains(adjEdge.getIndexA()) && pNodeSet.contains(adjEdge.getIndexB())) {
      ...
      for (int i = 0; i < interTable.getSize(); i++) {
        ...
        while (adjIt.hasNext()) {
          int[] adjArr = (int[]) adjIt.next();
          int adjJoinNode = adjArr[pairColumnA];
          if (interIns.getNode(joinNode) < adjJoinNode) break;
          if (interIns.getNode(joinNode) > adjJoinNode) continue;
          String adjIntr = adjArr[0] + "_" + adjArr[1];
          if (curInter.equals(adjIntr))
            //						interIns.setCycValid(true);
            joinedTable.getInsList().add(interIns);
        ...
        for (int i = 0; i < interTable.getSize(); i++) {
          ...
          while (it.hasNext()) {
            ...
            if (interNode == adjJoinNode) {
              ...
              } else joinedTable.joinAssign(interIns, addIdx, adjArr[pairColumnB]);
            }
          }
        }
      } else if (joinCount == qryGraph.getEdgeCount()) { // the last edge of the pattern
        //				HashMap<Integer, HashSet<Integer>> needMark = new HashMap<Integer,
        // HashSet<Integer>>();
  ...
  public ArrayList<int[]> getNodepairs(
      PatternEdges pe,
      HashMap<Integer, myNode> nodeAmap,
      double edgeLabel,
    ...
    for (Entry<Integer, myNode> entryA : nodeAmap.entrySet()) {
      ...
      HashMap<Integer, ArrayList<MyPair<Integer, Double>>> nodeAout =
          nodeA.getReachableWithNodes(); // indexes
      if (nodeAout == null) continue;
      ArrayList<MyPair<Integer, Double>> nodeAoutList = nodeAout.get(labelB);
      if (nodeAoutList == null) continue;
  ...
  public int getJoinNode(PatternEdges rEdge, PatternEdges curEdge) {
    ...
    } else if (rEdge.getIndexB() == curEdge.getIndexA()
        || rEdge.getIndexB() == curEdge.getIndexB()) {
