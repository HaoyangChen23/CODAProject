...
public class JoinTables {
  ...
  public InsTable joinTable(PatternEdges curEdge, InsTable scTables) {
    ...
    for (Entry<Integer, PatternEdges> entry : adjEdges.entrySet()) {
      ...
      int pairColloumnA = -1;
      int pairColloumnB = -1;
      if (adjEdge.getIndexA() == curEdge.getIndexA()
          || adjEdge.getIndexA() == curEdge.getIndexB()) {
        joinNode = adjEdge.getIndexA();
        addIdx = adjEdge.getIndexB();
        pairColloumnA = 0;
        pairColloumnB = 1;
      } else if (adjEdge.getIndexB() == curEdge.getIndexA()
          || adjEdge.getIndexB() == curEdge.getIndexB()) {
