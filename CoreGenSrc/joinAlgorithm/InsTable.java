/**
 * Project Name:PatKG_core_JoinOprV2.0 File Name:RETables.java Package Name:dataStructures Date:Dec
...
/**
 * ClassName:RETables Function: TODO ADD FUNCTION. Reason: TODO ADD REASON. Date: Dec 28, 2019
 *
 * @author JIAN
 * @version
 * @since JDK 1.6
...
public class InsTable {
  ...
  public void joinAssign(
      Instances ins, int addIdx, int addNode) { // (be added pnodeIdx, add graph node)
    Instances inscopy = ins.copy();
    inscopy.assign(addIdx, addNode);
    instanceList.add(inscopy);
  }

  public void reJoinAssign(
      HashSet<Integer> adjPatternIdx, int joinNode, Instances curIns, Instances adjIns) {
    ...
    for (int addIdx : adjPatternIdx) {
      if (addIdx != joinNode) {
        ...
        if (!curIns.contains(addNode)) { // has not been joined before
          if (inscopy == null) inscopy = curIns.copy();
          inscopy.assign(addIdx, addNode);
        } else return;
      }
    }
  ...
  public void reJoinAssignCycOld(
      HashSet<Integer> adjPatternIdx,
      ArrayList<Integer> joinNodes,
      Instances curIns,
      Instances adjIns) {
    Instances inscopy = null;
    for (int addIdx : adjPatternIdx) {
