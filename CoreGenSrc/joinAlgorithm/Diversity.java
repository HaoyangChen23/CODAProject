/**
 * Project Name:kFPD_opt File Name:DivEvaluation.java Package Name:joinAlgorithm Date:Jul 1, 2021
...
/**
 * ClassName:DivEvaluation Function: TODO ADD FUNCTION. Reason: TODO ADD REASON. Date: Jul 1, 2021
 *
 * @author JIAN
 * @version
 * @since JDK 1.6
...
public class Diversity<NodeType, EdgeType> {
  ...
  public boolean diversityCheck(
      SearchLatticeNode<NodeType, EdgeType> pat,
   ...
   *
   */
  public double divLowerBound(
      HashSet<Integer> fullCoverSet,
      HashSet<Integer> markedJoinSet,
   ...
   */
  public boolean coverVerticeSet(
    ...
    for (Entry<Integer, PatternEdges> entry : joinEdgeMap.entrySet()) {
      ...
      for (int i = 0; i < curInsTables.getSize(); i++) {
        ...
        HashMap<Integer, PatternEdges> midMap =
            (HashMap<Integer, PatternEdges>) fullEdgeMap.clone();
      ...
      if (divLowerBound(fullCoverSet, markedSet, upperDomainSet) >= Settings.t) return true;
    }
    // count the cover vertices
    for (Entry<Integer, HashSet<Integer>> e : coverVerticesMap.entrySet()) {
      HashSet<Integer> set = e.getValue();
  ...
  public void markNodesDiv(InsTable validTable, SearchLatticeNode<NodeType, EdgeType> pattern) {
    ...
    while (it.hasNext()) {
      ...
      for (int i = 0; i < qrySize; i++) {
        ...
        } else coverVerticesMap.get(i).add(ins.getNode(i));
      }
    }
    ...
    if (pNodeSet.contains(adjEdge.getIndexA()) && pNodeSet.contains(adjEdge.getIndexB())) {
      ...
      for (int i = 0; i < interTable.getSize(); i++) {
        ...
        while (adjIt.hasNext()) {
          ...
          if (curInter.equals(adjIntr)) joinedTable.getInsList().add(interIns);
        }
      }
