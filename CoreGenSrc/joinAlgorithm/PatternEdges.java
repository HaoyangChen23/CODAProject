...
public class PatternEdges implements Cloneable {
  ...
  public PatternEdges(
      int indexA, int labelA, int indexB, int labelB, double edgeLabel, int edgeIdx) {
    this.labelA = labelA;
    this.labelB = labelB;
    this.indexA = indexA;
    this.indexB = indexB;
  ...
  public InsTable getTuples(int needGetCount, int qrySize) {
    ...
    if (minMNInode == indexA && pointMap.size() != 0) { // has tuples need to get
      ...
      for (Iterator<java.util.Map.Entry<Integer, HashSet<Integer>>> pointIt =
              pointMap.entrySet().iterator();
          pointIt.hasNext(); ) {
  ...
  public void filter(
      ArrayList<int[]> needGetList,
      HashSet<Integer> traverseSet,
      HashSet<Integer> untraverseSet,
      int needGetCount) {
    for (Iterator<java.util.Map.Entry<Integer, HashSet<Integer>>> pointIt =
            pointMap.entrySet().iterator();
        pointIt.hasNext(); ) {
