...
public class Automorphism<NodeType, EdgeType> {
  ...
  public Automorphism(HPListGraph<NodeType, EdgeType> graph) {
    ...
    for (int i = 0; i < graph.getNodeCount(); i++) {
      ...
      for (IntIterator currentEdges = graph.getEdgeIndices(i); currentEdges.hasNext(); ) {
        ...
        if (direction == 1)
          currentNode.addreachableNode(
              otherNode, Double.parseDouble(graph.getEdgeLabel(edge) + ""));
        else if (direction == -1)
          otherNode.addreachableNode(
              currentNode, Double.parseDouble(graph.getEdgeLabel(edge) + ""));
      }
    }
    // now fill by label
    for (int i = 0; i < graph.getNodeCount(); i++) {
  ...
  public boolean hasAutomorphisms() {
    if (resultCounter == 1) return false;
    return true;
  }
}
