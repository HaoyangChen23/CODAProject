...
public class Util {
  ...
  public static ArrayList<MyPair<Integer, Double>> getIntersection(
      ArrayList<ArrayList<MyPair<Integer, Double>>> sets) {
    ArrayList<MyPair<Integer, Double>> inter = new ArrayList<MyPair<Integer, Double>>();
    if (sets.size() == 0) return inter;
    ...
    for (int i = 0; i < firstSet.size(); i++) {
      ...
      if (doesIntersect) inter.add(element);
    }
    return inter;
  }

  public static ArrayList<Integer> getIntersection(
      ArrayList<Integer> set1, ArrayList<Integer> set2) {
    ArrayList<Integer> inter = new ArrayList<Integer>();
    ArrayList<Integer> firstSet = set1;
    if (firstSet == null) return new ArrayList<Integer>();
    for (int i = 0; i < firstSet.size(); i++) {
      int element = firstSet.get(i);
      if (set2.contains(element)) inter.add(element);
    }
    return inter;
  }
    ...
    for (int i = 0; i < sets.size(); i++) {
      ...
      for (int j = i + 1; j < sets.size(); j++) {
        ...
        if (doesIntersect == false)
          points.add(
              new Point(
                  firstVariableCandidate.getVariableID(), secondVariableCandidate.getVariableID()));
      }
    }
    return points;
  }
  ...
  public static HashMap<Integer, HashSet<Integer>> clone(HashMap<Integer, HashSet<Integer>> in) {
    ...
    for (Iterator<Entry<Integer, HashSet<Integer>>> iterator = in.entrySet().iterator();
        iterator.hasNext(); ) {
