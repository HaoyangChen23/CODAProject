...
public class CommandLineParser {
  ...
  public static void parse(String[] args) {
    for (int i = 0; i < args.length; i++) {
      ...
      if (key.compareTo("filename") == 0) Settings.fileName = value;
      // get the core filename
      if (key.compareTo("coreFileName") == 0) Settings.coreFileName = value;
      // assign the datasets folder
      if (key.compareTo("datasetFolder") == 0) Settings.datasetsFolder = value;
      // assign top-k
      if (key.compareTo("k") == 0) Settings.k = Integer.parseInt(value);
      if (key.compareTo("t") == 0) Settings.t = Double.parseDouble(value);
      // approximate setting
      if (key.compareTo("approximate") == 0) {
        ...
        } else Settings.isApproximate = false;
      }
      ...
      // Distinct labels
      if (key.compareTo("distinct") == 0) Settings.DISTINCTLABELS = (value.compareTo("true") == 0);
      // caching substructures
      if (key.compareTo("caching") == 0) Settings.CACHING = (value.compareTo("true") == 0);
      // partial consistency
      if (key.compareTo("partial") == 0) Settings.isDecomposeOn = (value.compareTo("true") == 0);
      // limited time
      if (key.compareTo("limitedtime") == 0) Settings.LimitedTime = (value.compareTo("true") == 0);
    }
  }
}
