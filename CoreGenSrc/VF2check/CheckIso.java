...
public class CheckIso<NodeType, EdgeType> {
  ...
  public static <NodeType, EdgeType> boolean checkIso(
  ...
  private static boolean matchRecursivePlus(
      VF2State state, VF2Graph targetGraph, VF2Graph queryGraph) {
    if (state.depth == queryGraph.nodes.size()) { // found a match
      state.matched = true;
      return true;
    } else {
      ArrayList<VF2Pair<Integer, Integer>> candidatePairs =
          genCandidatePairs(state, targetGraph, queryGraph);
   ...
   */
  private static ArrayList<VF2Pair<Integer, Integer>> genCandidatePairs(
      VF2State state, VF2Graph targetGraph, VF2Graph queryGraph) {
  ...
  /**
   * Check the predecessor rule and successor rule It ensures the consistency of the partial
   * matching
   *
   * @param state VF2 State
   * @param targetNodeIndex Target Graph Node Index
