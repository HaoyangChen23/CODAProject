...
public class VF2State {
  ...
  public int[]
      core_1; // stores for each target graph node to which query graph node it maps ("-1" indicates
  // no mapping)
  public int[]
      core_2; // stores for each query graph node to which target graph node it maps ("-1" indicates
  // no mapping)

  public int[]
      in_1; // stores for each target graph node the depth in the search tree at which it entered
  // "T_1 in" or the mapping ("-1" indicates that the node is not part of the set)
  public int[]
      in_2; // stores for each query graph node the depth in the search tree at which it entered
  // "T_2 in" or the mapping ("-1" indicates that the node is not part of the set)
  public int[]
      out_1; // stores for each target graph node the depth in the search tree at which it entered
  // "T_1 out" or the mapping ("-1" indicates that the node is not part of the set)
  public int[]
      out_2; // stores for each query graph node the depth in the search tree at which it entered
  // "T_2 out" or the mapping ("-1" indicates that the node is not part of the set)

  public HashSet<Integer>
      T1in; // nodes that not yet in the partial mapping, that are the destination of branches start
  // from target graph
  public HashSet<Integer>
      T1out; // nodes that not yet in the partial mapping, that are the origin of branches end into
  // target graph
  public HashSet<Integer>
      T2in; // nodes that not yet in the partial mapping, that are the destination of branches start
  // from query graph
  public HashSet<Integer>
      T2out; // nodes that not yet in the partial mapping, that are the origin of branches end into
  // query graph
   ...
   */
  public VF2State(VF2Graph targetGraph, VF2Graph queryGraph) {
  ...
  public void extendMatch(int targetIndex, int queryIndex) {
    ...
    VF2Node targetNode = targetGraph.nodes.get(targetIndex);
    VF2Node queryNode = queryGraph.nodes.get(queryIndex);
  ...
  public void backtrack(int targetNodeIndex, int queryNodeIndex) {
    ...
    if (inT1in(targetNodeIndex)) T1in.add(targetNodeIndex);
    if (inT1out(targetNodeIndex)) T1out.add(targetNodeIndex);
    if (inT2in(queryNodeIndex)) T2in.add(queryNodeIndex);
    if (inT2out(queryNodeIndex)) T2out.add(queryNodeIndex);
