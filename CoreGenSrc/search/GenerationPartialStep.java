...
/**
 * This class defines the functionality of a single step during the generation of extensions for the
 * mining process
 *
 ...
 */
public abstract class GenerationPartialStep<NodeType, EdgeType>
    extends MiningStep<NodeType, EdgeType> {
  ...
  /**
   * tells this step, that it is to be used for the given <code>embedding</code> of the given <code>
   * node</code> this function has to call the next step, if necessary.
   *
   * @param node
   * @param embedding
   */
  ...
  /**
   * calls the next step for the given <code>embedding</code> of the given <code>node</code>
   *
   * @param node
   * @param embedding
   */
  ...
  //		final GenerationPartialStep<NodeType, EdgeType> gen = (GenerationPartialStep<NodeType,
  // EdgeType>) next;
  //		if (gen != null) {
  ...
  protected final void callNext(final SearchLatticeNode<NodeType, EdgeType> node) {
    @SuppressWarnings("unchecked")
    final GenerationPartialStep<NodeType, EdgeType> gen =
        (GenerationPartialStep<NodeType, EdgeType>) next;
    if (gen != null) {
      gen.call(node);
    }
  }

  /** tells this step that the extensions for the next node will be searched furthermore */
  public void reset() {
    resetNext();
  }
