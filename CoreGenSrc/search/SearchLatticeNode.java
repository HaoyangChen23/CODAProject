...
/**
 * This class defines the interface and basic functionality of a single node in a search lattice.
 *
 ...
 */
public abstract class SearchLatticeNode<NodeType, EdgeType>
    implements Generic<NodeType, EdgeType>, Serializable, serializableObject {
  ...
  /**
   * @param extension
   * @return a new node resulted by extending this node with the given <code>extension</code>
   */
  public abstract SearchLatticeNode<NodeType, EdgeType> extend(
      Extension<NodeType, EdgeType> extension);

  /**
   * release all internal structures to the local object pool that are never needed even if the node
   * is stored
   */
  ...
  /**
   * release all internal structures to the local object pool that are never needed if the node is
   * not stored
   */
