...
public class Searcher<NodeType, EdgeType> {
  ...
  public ArrayList<SearchLatticeNode<NodeType, EdgeType>> result;
  ...
  public Searcher(String graphPath, String corePath) throws Exception {
    ...
    UpperBoundOfMNI j = new UpperBoundOfMNI(kGraph, patternEdgeMap);
  ...
  public void initialize() {
    initials =
        new TreeMap<GSpanEdge<NodeType, EdgeType>, DFSCode<NodeType, EdgeType>>(
    ...
    for (Iterator<java.util.Map.Entry<Integer, HashMap<Integer, myNode>>> it =
      ...
      for (Iterator<myNode> iterator = tmp.values().iterator(); iterator.hasNext(); ) {
        ...
        HashMap<Integer, ArrayList<MyPair<Integer, Double>>> neighbours =
            node.getReachableWithNodes();
        if (neighbours != null)
          for (Iterator<Integer> iter = neighbours.keySet().iterator(); iter.hasNext(); ) {
            ...
            for (Iterator<MyPair<Integer, Double>> iter1 = neighbours.get(secondLabel).iterator();
                iter1.hasNext(); ) {
              MyPair<Integer, Double> mp = iter1.next();
              double edgeLabel = mp.getB();
              if (!freqEdgeLabels.contains(edgeLabel)) continue;
              ...
              if (!initials.containsKey(gedge)) {
                ...
                DFSCode<NodeType, EdgeType> code =
                    new DFSCode<NodeType, EdgeType>(sortedFrequentLabels, kGraph, null)
    ...
    for (final Iterator<Map.Entry<GSpanEdge<NodeType, EdgeType>, DFSCode<NodeType, EdgeType>>> eit =
  ...
  public ArrayList<SearchLatticeNode<NodeType, EdgeType>> search() {
    ...
    if (Settings.HMT) {
      qryExtender =
          extend.PatExtMeta(corePatternEdgeMap, maxDegreeMatrix, sortedNodeLabels, kGraph);
      initialize();
    ...
    if (corePattern.getPatternSize() == 1) { // the core pattern only has one node
      int coreNodeType = corePattern.getCoreNodeType();
      for (Entry<GSpanEdge<NodeType, EdgeType>, DFSCode<NodeType, EdgeType>> entry :
          initials.entrySet()) {
        DFSCode<NodeType, EdgeType> pattern = entry.getValue();
        String pString = DFScodeSerializer.serializePatternToString(pattern);
        int nodeLabelA = Integer.parseInt(pString.substring(0, pString.indexOf("_")));
        int nodeLabelB =
            Integer.parseInt(pString.substring(pString.indexOf("+") + 1, pString.length()));
        if (nodeLabelA == coreNodeType || nodeLabelB == coreNodeType) {
          IntFrequency freq = (IntFrequency) pattern.frequency(0);
          pattern.store(freq);
          initialsMap.put(pattern, freq);
        }
      }
      query = corePattern;
    } else {
      CheckIso c = new CheckIso(corePattern);
      ArrayList<SearchLatticeNode<NodeType, EdgeType>> initialList =
      ...
      IntFrequency MNIfreq = (IntFrequency) ((DFSCode<NodeType, EdgeType>) query).frequency(0);
      System.out.println("MNI of core pattern:" + MNIfreq);
    }
    qryWatch.stop();
    ...
    // Entrance
    EnumerateStrategy<NodeType, EdgeType> rs =
        new EnumerateStrategy<NodeType, EdgeType>(kGraph, patternEdgeMap);
    result = rs.search(extend, query, initialsMap);
    return result;
  }

  // generate the canonical core pattern
  public ArrayList<SearchLatticeNode<NodeType, EdgeType>> getCanonicalCore(
      ArrayList<SearchLatticeNode<NodeType, EdgeType>> list,
      Extender<NodeType, EdgeType> qryExtender) {
    ArrayList<SearchLatticeNode<NodeType, EdgeType>> retSet =
        new ArrayList<SearchLatticeNode<NodeType, EdgeType>>();
    Iterator it = list.iterator();
    while (it.hasNext()) {
      SearchLatticeNode<NodeType, EdgeType> pattern =
          (SearchLatticeNode<NodeType, EdgeType>) it.next();
      if (pattern.getHPlistGraph().getNodeCount() == corePattern.getHPlistGraph().getNodeCount()
          && pattern.getHPlistGraph().getEdgeCount()
              == corePattern.getHPlistGraph().getEdgeCount()) {
        ...
        final Collection<SearchLatticeNode<NodeType, EdgeType>> children =
            qryExtender.getChildren(pattern);
