...
public class Statistics<NodeType, EdgeType> {
  ...
  private void computeStats() {
    ...
    for (Iterator<Integer> iterator = distinctLabelsSize.keySet().iterator();
        iterator.hasNext(); ) {
      ...
      DistinctLabelStat stat =
          new DistinctLabelStat(
              distinctSize, localMaxNode, localMaxEdge, indexlocalMaxNode, indexlocalMaxEdge);
  ...
  private int getNumOfDistinctLabels(HPListGraph<NodeType, EdgeType> list) {
    ...
    for (int i = 0; i < list.getNodeCount(); i++) {
      ...
      if (!difflabels.contains(label)) difflabels.add(label);
    }
