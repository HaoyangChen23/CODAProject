...
public class Decomposer<NodeType, EdgeType> {
  ...
  ArrayList<HashMap<HPListGraph<NodeType, EdgeType>, ArrayList<Integer>>>
      mappings; // graph ~ node mappings
  ...
  public void decompose() {
    ...
    for (int i = 0; i < currentGraph.getEdgeCount(); i++) {
      ...
      int currentEdge = i;
      HPListGraph<NodeType, EdgeType> newGraph =
          (HPListGraph<NodeType, EdgeType>) currentGraph.clone();
      newGraph.removeEdge(currentEdge);
      HashMap<HPListGraph<NodeType, EdgeType>, ArrayList<Integer>> currentEdgeMapping =
          new HashMap<HPListGraph<NodeType, EdgeType>, ArrayList<Integer>>();
      // Now pass by each node !!
      for (int j = 0; j < newGraph.getNodeCount(); j++) {

        HPListGraph<NodeType, EdgeType> connectedComp =
            new HPListGraph<NodeType, EdgeType>(); // new connected component!!
        ArrayList<Integer> mapping = new ArrayList<Integer>(); // corresponding mapping !!

        int nodeID = j; // start from here and search !!
        if (colored[j] == 1) continue;
        ...
        while (!DFSstack.isEmpty()) {
          ...
          if (colored[currentNodeIDmapping] == 1) continue;
          ...
          for (IntIterator it = newGraph.getEdgeIndices(currentNodeIDmapping); it.hasNext(); ) {
            ...
            if (colored[otherNodeMapping] == 1) continue;
