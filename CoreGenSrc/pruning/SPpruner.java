...
public class SPpruner {
  ...
  public <EdgeType> void getPrunedLists(
      HashMap<Integer, HashMap<Integer, myNode>> nodesByLabel, Query qry) {
    System.out.println("called Automorphism pruned lists");
    HashMap<Integer, HashMap<Integer, myNode>> pruned =
        new HashMap<Integer, HashMap<Integer, myNode>>(); // QueryID -> NodeID->NODE
    ArrayList<ConnectedComponent> cls = qry.getConnectedLabels();
    ...
    HashMap<Integer, HashMap<Integer, Integer>> nodeOutLabelDegrees =
        new HashMap<Integer, HashMap<Integer, Integer>>(); // nodeID-->(Label,Degree)
    HashMap<Integer, HashMap<Integer, Integer>> nodeInLabelDegrees =
    ...
    for (int i = 0; i < cls.size(); i++) {
      ...
      if (degreeA == null) degreeA = 0;
      Integer degreeB = nodeBmap.get(c.getLabelA());
      if (degreeB == null) degreeB = 0;
    ...
    for (int i = 0; i < qry.getListGraph().getNodeCount(); i++) {
      ...
      for (Iterator<Entry<Integer, myNode>> it = candidates.entrySet().iterator(); it.hasNext(); ) {
        ...
        if (degreeOutCons != null)
          for (Iterator<Entry<Integer, Integer>> iterator = degreeOutCons.entrySet().iterator();
              iterator.hasNext(); ) {
        ...
        if (isValidNode && degreeInCons != null) {
          for (Iterator<Entry<Integer, Integer>> iterator = degreeInCons.entrySet().iterator();
              iterator.hasNext(); ) {
            Entry<Integer, Integer> entry = iterator.next();
            int label = entry.getKey();
            int degree = entry.getValue();
        ...
        if (isValidNode == false) it.remove();
      }
    }
  ...
  public <EdgeType> void getPrunedLists(
      Graph graph, Query qry, HashMap<Integer, HashSet<Integer>> nonCandidates) {
    ...
    //		for (Iterator<Entry<Integer, HashSet<Integer>>> iterator =
    // nonCandidates.entrySet().iterator(); iterator.hasNext();)
    //		{
  ...
  public static void printSPs(HashMap<Integer, HashMap<Integer, myNode>> pruned) {
    ...
    for (Iterator<java.util.Map.Entry<Integer, HashMap<Integer, myNode>>> it =
            pruned.entrySet().iterator();
        it.hasNext(); ) {
