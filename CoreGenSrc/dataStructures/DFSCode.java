...
// Import TED's DFS class

/**
 * Implements the SimpleDFSCode that represents a subgraph during the search.
 *
 * <p>It can/will be stored in local object pool to avoid object generation/garbage collection.
 *
...
public class DFSCode<NodeType, EdgeType> extends SearchLatticeNode<NodeType, EdgeType>
    implements Comparable<DFSCode<NodeType, EdgeType>>,
  ...
  public SearchLatticeNode<NodeType, EdgeType> extend(
    ...
    assert extension instanceof GSpanExtension
        : "SimpleDFSCode.extend(..) is just applicable for GSpanExtensions";
    ...
    return new DFSCode<NodeType, EdgeType>(
            sortedFreqLabels, singleGraph, new HashMap<Integer, HashSet<Integer>>())
  ...
  public IntFrequency upperBound() {
    ...
    ConstraintGraph cg =
        new ConstraintGraph(singleGraph, qry, (HashMap<Integer, HashSet<Integer>>) nonCandidates);
  ...
  public final Frequency frequency(int minFreq) {
    ...
    if (finalFrequency != null) return finalFrequency;
    else {
      if (nonCandidates == null) {
        ...
        if (Settings.CACHING) {
          ...
          com.decompose();
          ArrayList<HashMap<HPListGraph<NodeType, EdgeType>, ArrayList<Integer>>> maps =
              com.getMappings();
          for (int i = 0; i < maps.size(); i++) // iterate over edges removed!!
          {
            ...
            for (Iterator<Entry<HPListGraph<NodeType, EdgeType>, ArrayList<Integer>>> iterator =
                    edgeRemoved.entrySet().iterator();
                iterator.hasNext(); ) {
              Entry<HPListGraph<NodeType, EdgeType>, ArrayList<Integer>> removedEdgeEntry =
                  iterator.next();
              ...
              if (DfscodesCache.cache.containsKey(key)) {
                ...
                HashMap<Integer, HashSet<Integer>> nodeNonCandidates =
                    DfscodesCache.cache.get(key); // node ~ noncandidates
        ...
        ConstraintGraph cg =
            new ConstraintGraph(singleGraph, q, (HashMap<Integer, HashSet<Integer>>) nonCandidates);
        DFSSearch df = new DFSSearch(cg, minFreq, nonCandidates);
        df.searchExistances();
        currentVariables = df.getResultVariables();
        int freq = df.getFrequencyOfPattern();
        ...
        if (Settings.CACHING) {
          if (freq >= minFreq) nonCandidates = df.getNonCandidates();
        }

        if (Settings.CACHING) {
          String code = me.toString();
          if (code == null) System.out.println("string null");
          HashMap<Integer, HashSet<Integer>> nonCands = nonCandidates;
          if (nonCands == null) System.out.println("nonvands null");
        ...
        if (Settings.CACHING) {
          ...
          for (int i = 0; i < maps.size(); i++) // iterate over edges removed!!
          {
            ...
            for (Iterator<Entry<HPListGraph<NodeType, EdgeType>, ArrayList<Integer>>> iterator =
              ...
              Entry<HPListGraph<NodeType, EdgeType>, ArrayList<Integer>> removedEdgeEntry =
                  iterator.next();
              HPListGraph<NodeType, EdgeType> listGraph =
                  removedEdgeEntry.getKey(); // each graph candidate
              String key = listGraph.toString();
    ...
    for (int i = graph.getDegree(gNodeA) - 1; i >= 0; --i) {
      ...
      if (usedEdges[edge] == UNUSED) {
        ...
        final MinExtension<NodeType, EdgeType> next =
            new MinExtension<NodeType, EdgeType>()
                .set(
                    nodeA,
                    usedNodes[gNodeB],
                    ...
                    graph.getDirection(edge, gNodeA),
                    edge,
                    gNodeB,
                    thelabelA,
                    thelabelB);
   ...
   *
   * <p>search for a smaller first edge or starts recursive extension for equally ones
   *
   * @param set
   * @param ackNodes
   * @param usedNodes
  ...
  private boolean isCan(
      final MinExtensionSet<NodeType, EdgeType> set,
      final int[] ackNodes,
      final int[] usedNodes,
      final int[] usedEdges) {
    ...
    for (int node = hp.getMaxNodeIndex() - 1; node >= 0; --node) {
      ...
      if (foundEdge > -2 && ack != null) { // node is a start node
        final MinExtension<NodeType, EdgeType> exts =
            getExtensions(0, hp, node, usedEdges, usedNodes);
        set.addAll(exts);
   ...
   * helper function for the test of beeing canonical
   *
   * <p>extends current detected SimpleDFSCode and searches for new extensions
   *
   * @param ackEdge
   * @param set
   * @param lastNode
  ...
  private boolean isCan2(
      final GSpanEdge<NodeType, EdgeType> ackEdge,
      final MinExtensionSet<NodeType, EdgeType> set,
      final int lastNode,
      final int[] usedNodes,
      final int[] usedEdges,
    ...
    for (MinExtension<NodeType, EdgeType> ack = first;
        ack.compareTo(first) == 0;
        ack = ack.forward) {
      ...
      if (usedEdges[ack.gEdgei] != UNUSED) {
        ...
        if (ack.getNodeB() == UNUSED) { // forward edge
          ...
          final MinExtension<NodeType, EdgeType> exts =
              getExtensions(lastNode + 1, graph, ack.gNodeBi, usedEdges, usedNodes);
          set.addAll(exts);
          // recursiv search
