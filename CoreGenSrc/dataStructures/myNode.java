...
public class myNode {
  ...
  private HashMap<Integer, ArrayList<MyPair<Integer, Double>>>
      reachableNodes; // represented by Label~<nodeID,edge_label>, represents the outgoing nodes

  private HashMap<Integer, ArrayList<MyPair<Integer, Double>>>
      reachedBYNodes; // represented by Label~<nodeID,edge_label>, represents the ingoing nodes
  ...
  public int getOutDegree(int label) {
    if (reachableNodes == null) return 0;
    if (reachableNodes.get(label) == null) return 0;
    return reachableNodes.get(label).size();
  }

  public List getAEdgeLabelsList(int label) {
    if (reachableNodes == null) return null;
    if (reachableNodes.get(label) == null) return null;
  ...
  public List getBEdgeLabelsList(int label) {
    if (reachedBYNodes == null) return null;
    if (reachedBYNodes.get(label) == null) return null;
  ...
  public int getinDegree(int label) {
    if (reachedBYNodes == null) return 0;
    if (reachedBYNodes.get(label) == null) return 0;
  ...
  public void addreachableNode(myNode node, double edgeLabel) {
    ...
    if (!list.contains(node.getID())) list.add(new MyPair(node.getID(), edgeLabel));
  ...
  public void printOutReachableNodes() {
    ...
    for (Iterator<ArrayList<MyPair<Integer, Double>>> iterator = reachableNodes.values().iterator();
        iterator.hasNext(); ) {
   ...
   */
  public void setReachableNodes_1hop(
      Graph graph, HashMap<Integer, HashMap<Integer, myNode>> freqNodesByLabel) {
  ...
  public void setReachableNodes_1hopNew(
      Graph graph, HashMap<Integer, HashMap<Integer, myNode>> freqNodesByLabel, int kthFreq) {
    // get edge for each node
    IntIterator it = graph.getListGraph().getOutEdgeIndices(getID());
    for (; it.hasNext(); ) {
      int edge = it.next();
  ...
  public boolean hasReachableNodes() {
    if (reachableNodes == null) return false;
    else return true;
  }

  public boolean isWithinTheRangeOf(int NodeIndex, int nodeLabel) {
    if (reachableNodes.get(nodeLabel) == null) return false;
