...
/**
 * Represents the edge tuples used in gSpan to represent one edge in the DFS-Code.
 *
 ...
 */
public class GSpanEdge<NodeType, EdgeType>
    implements Comparable<GSpanEdge<NodeType, EdgeType>>,
        Cloneable<GSpanEdge<NodeType, EdgeType>>,
  ...
  public final void addTo(final HPMutableGraph<NodeType, EdgeType> graph) {
    ...
    graph.addEdgeIndex(nodeA, nodeB, (EdgeType) (edgeLabel + ""), direction);
  }

  @Override
  public GSpanEdge<NodeType, EdgeType> clone() {
    return new GSpanEdge<NodeType, EdgeType>()
        .set(nodeA, nodeB, labelA, edgeLabel, labelB, direction, ThelabelA, ThelabelB);
  }
   ...
   * @return <0; 0; or >0
   */
  public final int compareTo(final GSpanEdge<NodeType, EdgeType> other, final int nodeB) {
  ...
  /**
   * calculates the edge of th given graph that corresponds to this gSpanEdge in the given
   * "embedding"
   *
   * @param graph
   * @param ackNodes
   ...
   */
  public final int getCorresponding(final HPGraph<NodeType, EdgeType> graph, final int[] ackNodes) {
   ...
   */
  public GSpanEdge<NodeType, EdgeType> set(
      final int nodeA,
      final int nodeB,
      final int labelA,
      final int edgeLabel,
    ...
    if (direction == Edge.UNDIRECTED && nodeA == 0 && nodeB == 1 && labelA > labelB) {
      this.labelA = labelB;
      this.labelB = labelA;
  ...
  public String toString() {
    return "(" + nodeA + " " + nodeB + ": " + labelA + " " + edgeLabel + " " + labelB + " "
        + direction + ")";
  }
}
