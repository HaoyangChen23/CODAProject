...
public class Graph {
  ...
  public int[][] getMaxDegreeMatrix() {
    ...
    int index =
        0; // node label index, so the data should pre-processing as node label by increasing order
    // one by one
    while (iter.hasNext()) {
  ...
  public Graph(int ID) {
    ...
    if (StaticData.hashedEdges != null) {
      ...
      System.out.println(
          StaticData.hashedEdges.hashCode()); // throw exception if more than one graph was created
    }
  ...
  public <NodeType, EdgeType> void loadFromFile_Ehab(String fileName) throws Exception {
    ...
    if (tempLine.charAt(0) == 'e') line = tempLine;
    else line = rows.readLine();
    ...
    for (Iterator<java.util.Map.Entry<Double, Integer>> it =
            this.edgeLabelsWithFreq.entrySet().iterator();
        it.hasNext(); ) {
    ...
    for (Iterator<java.util.Map.Entry<Integer, HashMap<Integer, myNode>>> it =
            nodesByLabel.entrySet().iterator();
        it.hasNext(); ) {
  ...
  public static HashMap<Integer, ArrayList<Integer>> sortByKeyIncreasing(
      HashMap<Integer, ArrayList<Integer>> nodeIndexMap) {
    // sort by increasing
    List<Map.Entry<Integer, ArrayList<Integer>>> list =
        new ArrayList<Map.Entry<Integer, ArrayList<Integer>>>(nodeIndexMap.entrySet());
    Collections.sort(
        list,
        new Comparator<Map.Entry<Integer, ArrayList<Integer>>>() {
          @Override
          public int compare(
              Map.Entry<Integer, ArrayList<Integer>> o1,
  ...
  public void printFreqNodes() {
    for (Iterator<java.util.Map.Entry<Integer, HashMap<Integer, myNode>>> it =
            freqNodesByLabel.entrySet().iterator();
        it.hasNext(); ) {
      java.util.Map.Entry<Integer, HashMap<Integer, myNode>> ar = it.next();
  ...
  public int addEdge(int nodeA, int nodeB, double edgeLabel) {
    ...
    if (I == null) edgeLabelsWithFreq.put(edgeLabel, 1);
    else edgeLabelsWithFreq.put(edgeLabel, I.intValue() + 1);
