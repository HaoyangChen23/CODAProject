...
public class CompleteQueryPattern<NodeType, EdgeType> {
  ...
  public DFSCode<NodeType, EdgeType> loadFromFile_Core(String fileName) throws Exception {
    ...
    if (line != null) {
      do {
        final String[] parts = line.split("\\s+");
        if (parts.length < 3) { // only has one node
          pattern.setSize(1);
          return pattern;
        }
        ...
        final GSpanEdge<NodeType, EdgeType> gEdge =
            new GSpanEdge<NodeType, EdgeType>()
                .set(index1, index2, labelA, edgeLabel, labelB, 1, firstLabel, secondLabel);
        // first line
        if (edgeNum == 0) {
          ...
          gEdge.addTo(lg);
          pattern =
              new DFSCode<NodeType, EdgeType>(sortedNodeLabels, kGraph, null)
