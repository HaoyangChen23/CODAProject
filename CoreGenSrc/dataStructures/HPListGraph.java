 ...
 *
 * <p>It stores the required node and edge informations in different int-arrays.
 *
...
public class HPListGraph<NodeType, EdgeType> implements HPMutableGraph<NodeType, EdgeType> {
  ...
  /**
   * stores the first deleted Edge/Node ([0/1]), the node/edge count([2/3]), and the max-node/edge
   * index([4/5])
   */
  final int[] status;

  /**
   * stores for each node in the array[nodeIndex] the degree ([x][0]), the in/outdegree ([x][1/2])
   * and the connected edge [x][3..].
   *
   * <p>It also contains the single linked deleted list at [x][NEXTEMPTY] in each deleted node. -1
   * marks the end of this list.
   */
  int[][] node_edges;

  /**
   * stores the first node of each edge at [edgeIdx*3+NODEA], the second node at [edgeIdx*3+NODEB]
   * and the direction in respect to nodeA at [edgeIdx*3+DIRECTION].
   *
   * <p>It also contains the single linked deleted list at [edgeIdx*3+NEXTEMPTY] in each deleted
   * edge. -1 marks the end of the list.
   */
  ...
  private HPListGraph(final String name, final int id, final int nodeSize, final int edgeSize) {
    this.name = name;
    this.id = id;
  ...
  @SuppressWarnings("unchecked")
  private final int _addEdge(final int na, final int nb, final EdgeType label, int direction) {
    ...
    if (idx < 0) { // no empty edge available
      ...
      if (idx >= length) { // resize nodes
        ...
        System.arraycopy(
            edge_nodes_and_direction,
            0,
            edge_nodes_and_direction = new int[3 * newlength],
            0,
            3 * length);
        System.arraycopy(
            edge_labels, 0, edge_labels = ((EdgeType[]) new Object[newlength]), 0, length);
      }
    } else {
  ...
  private final void _addEdgeToNode(final int ni, final int ei, final int direction) {
  ...
  private final int _addNode(final NodeType label) {
    ...
    if (idx < 0) { // no empty nodes available
      ...
      if (idx >= length) { // resize nodes
        final int newlength = (int) (RESIZE_SCALE * idx);
        System.arraycopy(node_edges, 0, node_edges = new int[newlength][], 0, length);
        System.arraycopy(
            node_labels, 0, node_labels = ((NodeType[]) new Object[newlength]), 0, length);
    ...
    nodesChanged();
    assert node_edges[0] != null : dumpGraph("_addNode(" + label.toString() + ") " + idx);
    return idx;
  }
  ...
  final void _deleteEdge(final int ei) {
    _deleteEdgeFromNode(
        ei, edge_nodes_and_direction[ei * 3 + NODEA], edge_nodes_and_direction[ei * 3 + DIRECTION]);
    _deleteEdgeFromNode(
        ei,
        edge_nodes_and_direction[ei * 3 + NODEB],
  ...
  private final void _deleteEdgeFromNode(final int ei, final int ni, final int direction) {
   ...
   */
  @Override
  public int addEdgeIndex(
      final int nodeAIdx, final int nodeBIdx, final EdgeType label, final int direction) {
   ...
   */
  @Override
  public int addNodeAndEdgeIndex(
      final int nodeAIdx, final NodeType nodeLabel, final EdgeType edgeLabel, final int direction) {
  ...
  public HPGraph<NodeType, EdgeType> clone() {
    ...
    final HPListGraph<NodeType, EdgeType> ret =
        new HPListGraph<NodeType, EdgeType>(name, ID++, ns, es);
    System.arraycopy(this.status, 0, ret.status, 0, this.status.length);

    assert this.node_edges.length == ns
        : dumpGraph("clone(): node_edges.length wrong " + this.node_edges.length + " " + ns);
    assert ret.node_edges.length == ns
        : ret.dumpGraph("clone(): node_edges.length wrong " + ret.node_edges.length + " " + ns);

    for (int i = ns - 1; i >= 0; --i) {
      ret.node_edges[i] = new int[this.node_edges[i].length];
      System.arraycopy(this.node_edges[i], 0, ret.node_edges[i], 0, this.node_edges[i].length);
    }
    System.arraycopy(this.edge_nodes_and_direction, 0, ret.edge_nodes_and_direction, 0, 3 * es);
    ...
    assert ret.node_edges[0] != null
        : dumpGraph("clone(): original") + "\n" + ret.dumpGraph("clone(): clone");
  ...
  public IntIterator edgeIndexIterator() {
    return new IntIterator() {
      ...
      public boolean hasNext() {
        ...
        for (next = last + 1; next < max && edge_labels[next] == DELETED; ++next) {
  ...
  public int getDirection(final int edgeIdx, final int nodeIdx) {
    ...
    throw new IllegalArgumentException(
        "node index " + nodeIdx + " is invalid for the edge " + edgeIdx + "!");
  }
  ...
  public int getEdge(final int nodeAIdx, final int nodeBIdx) {
    ...
    for (int i = nodeA[DEGREE] + FIRSTEDGE - 1; i >= FIRSTEDGE; --i) {
      ...
      final int Dir = this.edge_nodes_and_direction[edgeIdx * 3 + DIRECTION];
  ...
  public EdgeType[] getEdgeLabelList() {
  ...
  public IntIterator getInEdgeIndices(final int nodeIdx) {
    return new IntIterator() {
      ...
      public boolean hasNext() {
        ...
        for (next = last + 1;
            next < node[DEGREE] + FIRSTEDGE && getDirection(node[next], nodeIdx) != Edge.INCOMING;
            ++next) {
  ...
  public IntIterator getOutEdgeIndices(final int nodeIdx) {
    return new IntIterator() {
      ...
      public boolean hasNext() {
        ...
        for (next = last + 1;
            next < node[DEGREE] + FIRSTEDGE && getDirection(node[next], nodeIdx) != Edge.OUTGOING;
            ++next) {
  ...
  public IntIterator nodeIndexIterator() {
    return new IntIterator() {
      ...
      public boolean hasNext() {
        ...
        for (next = last + 1; next < max && node_labels[next] == DELETED; ++next) {
