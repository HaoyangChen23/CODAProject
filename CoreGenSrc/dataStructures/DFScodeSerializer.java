...
public class DFScodeSerializer {
  ...
  public static String serialize(final HPListGraph graph) {
    ...
    for (int edgeIdx = edges.nextSetBit(0); edgeIdx >= 0; edgeIdx = edges.nextSetBit(edgeIdx + 1)) {
      ...
      if (graph.getDirection(edgeIdx) >= 0) {
        text += "e " + node1 + " " + node2 + " " + edgeLabel + "\n";
      } else {
        text += "e " + node2 + " " + node1 + " " + edgeLabel + "\n";
      }
    }
  ...
  public static <NodeType, EdgeType> String serializePatternToString(
      SearchLatticeNode<NodeType, EdgeType> pattern) {
    String returnString = "";
    HPListGraph midGraph = pattern.getHPlistGraph();
    BitSet nodes = midGraph.getNodes();
    BitSet edges = midGraph.getEdges();
    List<Integer> nodeLabelList = new ArrayList<Integer>();
    for (int nodeIdx = nodes.nextSetBit(0); nodeIdx >= 0; nodeIdx = nodes.nextSetBit(nodeIdx + 1)) {
      int label = (Integer) midGraph.getNodeLabel(nodeIdx);
      nodeLabelList.add(label);
    }
    int edgeLabel = 0;
    for (int edgeIdx = edges.nextSetBit(0); edgeIdx >= 0; edgeIdx = edges.nextSetBit(edgeIdx + 1)) {
      edgeLabel = Integer.parseInt((String) midGraph.getEdgeLabel(edgeIdx));
    }
    int labelA = nodeLabelList.get(0);
    int labelB = nodeLabelList.get(1);
    returnString = labelA + "_" + edgeLabel + "+" + labelB;
