...
/**
 * This interface represents a HighPerformance Graph which avoids costly object generation by
 * indexing.
 *
 * <p>The transformation from normal Graph to HPGraph is straight forward, so development with
 * objects and afterwards transformation to higher performance is possible.
 *
 * <p>There are at least consistency checks as possible, so correct usage is necessary!
 *
 ...
 */
public interface HPGraph<NodeType, EdgeType>
    extends Cloneable<HPGraph<NodeType, EdgeType>>, Generic<NodeType, EdgeType>, Serializable {
   ...
   * @throws IllegalArgumentException if the node is not a node of the edge
   */
  public int getDirection(final int edgeIdx, final int nodeIdx) throws IllegalArgumentException;

  /**
   * @param nodeAIdx the index of the start node
   * @param nodeBIdx the index of the end node
   * @return the index of the edge started in nodeA and ended in nodeB, or -1 if none is available
   */
  ...
  /**
   * @param nodeIdx the index of the node
   * @return an iterator over all edges (indices) connected with the given node
   */
  ...
  /**
   * @return a BitSet which setted bits describe that the corresponding edge is available in the
   *     HPGraph
   */
  ...
  /**
   * @param nodeIdx the index of the node
   * @return an iterator over all directed incoming edges (indices) connected with the given node
   */
  ...
  /**
   * @return a BitSet which setted bits describe that the corresponding node is available in the
   *     HPGraph
   */
  ...
  /**
   * @param nodeIdx the index of the node
   * @return an iterator over all directed outgoing edges (indices) connected with the given node
   */
   ...
   *
   * @param edgeIdx the index of the node
